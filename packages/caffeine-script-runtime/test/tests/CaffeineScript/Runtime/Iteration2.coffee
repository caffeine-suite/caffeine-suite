...
defineModule module, suite:
  ...
  array: ->
    ...
    test "array v, k from a: 1 b: 2 c: 3 with k", ->
      bar = Caf.array objectAbc, null, (v, k) -> k
      assert.eq bar, ["a", "b", "c"]

    test "array v, k from [1, 2, 3] with k", ->
      bar = Caf.array array123, null, (v, k) -> k
      assert.eq bar, [0, 1, 2]

    test "array123/objectAbc: array a from ... with a * 2", ->
      expected = (a * 2 for a in array123)
      assert.eq expected, Caf.array array123,  null, (a) -> a * 2
      assert.eq expected, Caf.array objectAbc, null, (a) -> a * 2

    test "array123/objectAbc: array ... into [] 4 5 6", ->
      expected = [4, 5, 6, 1, 2, 3]
      assert.same into = [4, 5, 6], Caf.array array123, into
      assert.eq expected, Caf.array array123,  [4, 5, 6]
      assert.eq expected, Caf.array objectAbc, [4, 5, 6]

    test "array123/objectAbc: array a from ... when a % 2 > 0", ->
      expected = [1, 3]
      assert.eq expected, Caf.array array123,  null, null, (a) -> a % 2 > 0
      assert.eq expected, Caf.array objectAbc, null, null, (a) -> a % 2 > 0

    test "array123/objectAbc: array v, k from ... with v", ->
      expected = [1, 2, 3]
      assert.eq expected, Caf.array array123,  null, (v, k) -> v
      assert.eq expected, Caf.array objectAbc, null, (v, k) -> v
  ...
  object: ->
    ...
    test "object v, k from a: 1 b: 2 c: 3 with k", ->
      assert.eq a:"a", b:"b", c:"c",  Caf.object objectAbc, null, (v, k) -> k

    test "object v, k from [1, 2, 3] with k", ->
      assert.eq 1:0, 2:1, 3:2,        Caf.object array123, null, (v, k) -> k

    test "array123/objectAbc: object a from ... with a * 2", ->
      assert.eq 1:2, 2:4, 3:6, Caf.object array123,  null, (a) -> a * 2
      assert.eq a:2, b:4, c:6, Caf.object objectAbc, null, (a) -> a * 2

    test "array123/objectAbc: object ... into d:4, e:5, f:6", ->
      assert.same into = d:4, e:5, f:6,       Caf.object array123,  into
      assert.eq d:4, e:5, f:6, 1:1, 2:2, 3:3, Caf.object array123,  d:4, e:5, f:6
      assert.eq d:4, e:5, f:6, a:1, b:2, c:3, Caf.object objectAbc, d:4, e:5, f:6

    test "array123/objectAbc: object a from ... when a % 2 > 0", ->
      assert.eq 1:1, 3:3, Caf.object array123,  null, null, (a) -> a % 2 > 0
      assert.eq a:1, c:3, Caf.object objectAbc, null, null, (a) -> a % 2 > 0

    test "array123/objectAbc: object v, k from ... with v", ->
      assert.eq 1:1, 2:2, 3:3, Caf.object array123,  null, (v, k) -> v
      assert.eq a:1, b:2, c:3, Caf.object objectAbc, null, (v, k) -> v
  ...
  arrayRange: ->
    # Caf.arrayRange: (fromValue, toValue, byValue, into = [], withClause = returnFirst, whenClause = returnTrue) ->
    ...
    test "array from 10 to 5 by 2",   -> assert.eq [],          Caf.arrayRange 10, 5, 2
    test "array from 10 to 5 by -2",  -> assert.eq [10, 8, 6],  Caf.arrayRange 10, 5, -2
    test "array from 5 to 10 by 2",   -> assert.eq [5, 7, 9],   Caf.arrayRange 5, 10, 2
    test "array from 5 to 10 by -2",  -> assert.eq [],          Caf.arrayRange 5, 10, -2

    test "array v to 3 with v * 2",   ->
      assert.eq [0, 2, 4, 6], Caf.arrayRange 0, 3, null, null, (v) -> v * 2

    test "array v to 3 when v %% 3 == 0",   ->
      assert.eq [0, 3], Caf.arrayRange 0, 3, null, null, null, (v) -> v %% 3 == 0

    test "array to 3 into [4, 5, 6]",   ->
      assert.eq [4, 5, 6, 0, 1, 2, 3], Caf.arrayRange 0, 3, null, [4, 5, 6]

    test "array from 1 til 3",        -> assert.eq [1,  2               ],    Caf.arrayRange 1, 3,  null, null, null, null, true
    test "array til 3",               -> assert.eq [0,  1,  2           ],    Caf.arrayRange 0, 3,  null, null, null, null, true
    test "array til -3",              -> assert.eq [0, -1, -2           ],    Caf.arrayRange 0, -3, null, null, null, null, true
