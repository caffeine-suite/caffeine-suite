...
module.exports =
  ...
  generateSourceMapParseTest: (name, source, options) ->
    ...
    niceTest "#{name}: #{source}", ->
      try
        {parseTree, semanticTree, transformedSemanticTree, compiled:{js:withSourceMapJs}} =
        ...
        if error.info
          {parseTree, semanticTree, transformedSemanticTree} = error.info
        log "\nFAIL: #{name}".red
